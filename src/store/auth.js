...
export default {
    ...
    actions: {
        async getRegisterData ({ commit, dispatch }, payload) {
            const APIkey = "AIzaSyCxsO-u5OHNv5GhyM3NvGguGiy7emlUlRE";
            ...
            try {
                ...
                const newUserData = {
                    userId: data.localId, fullname: payload.fullname,
        ...
        async addNewUser({ commit, state }, payload) {
            try {
                ...
                (
                    `https://vintage-project-c5980-default-rtdb.firebaseio.com/user.json?auth=${state.token}`, payload
                );
        ...
        async getUser({ commit }, payload) {
            try {
                ...
                (
                    `https://vintage-project-c5980-default-rtdb.firebaseio.com/user.json`
                )
        ...
        async updateUser({ commit, state }, payload) {
            try {
                await axios.put(
                    `https://vintage-project-c5980-default-rtdb.firebaseio.com/user/${state.userLogin.userId}.json?auth=${state.token}`, payload
                );
                commit("setUserLogin", {userData: payload, loginStatus: true });
            } catch(err) {
                console.log(err);
